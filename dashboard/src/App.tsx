...
export default function App(){
  ...
  async function runAnalysis(){setBusy('Building deterministic incident packet');setError('');setSimulation(null);setActive(null);try{const data=await json<Analysis>('/api/v1/analyses',{method:'POST',body:JSON.stringify({scenario_id:scenario,source:'fixture'})});setAnalysis(data);setSelected(data.candidates.find(c=>c.state==='validated')?.policy_id||data.candidates[0]?.policy_id||null)}catch(e){setError(String(e))}finally{setBusy('')}}
  async function refreshRecommendation(){if(!analysis)return;setBusy('Loading latest Codex MCP submission');setError('');try{const data=await json<Analysis>(`/api/v1/analyses/${analysis.incident_id}`);setAnalysis(data);setSimulation(null);setSelected(data.candidates.find(c=>c.state==='validated')?.policy_id||data.candidates[0]?.policy_id||null)}catch(e){setError(String(e))}finally{setBusy('')}}
  async function rankAll(){if(!analysis)return;setBusy('Simulating and ranking all valid candidates');setError('');try{const ranked=await json<Simulation[]>(`/api/v1/incidents/${analysis.incident_id}/rank`,{method:'POST'});if(!ranked.length)throw new Error('No valid candidates to rank');setSelected(ranked[0].policy_id);setSimulation(ranked[0]);setAnalysis({...analysis,candidates:analysis.candidates.map(c=>c.state==='validated'?{...c,state:'simulated'}:c)})}catch(e){setError(String(e))}finally{setBusy('')}}
  ...
  async function reset(){setBusy('Resetting deterministic demo');try{await json('/api/v1/demo/reset',{method:'POST'});setAnalysis(null);setSelected(null);setSimulation(null);setActive(null);setError('')}catch(e){setError(String(e))}finally{setBusy('')}}
  return <main><header><div><span className="eyebrow">TRACE-AWARE CONTROL PLANE</span><h1>SLO <i>Guardian</i></h1></div><div className="controls"><span className="mcp-badge">LOCAL MCP · NO API KEY</span><select value={scenario} onChange={e=>setScenario(e.target.value)}>{scenarios.map(s=><option value={s[0]} key={s[0]}>{s[1]}</option>)}</select>{analysis&&<button className="secondary" onClick={refreshRecommendation}>Refresh MCP result</button>}{analysis&&<button className="secondary" onClick={reset}>Reset</button>}<button onClick={runAnalysis} disabled={!!busy}>Analyze incident</button></div></header>
    {busy&&<div className="notice pulse">{busy}…</div>}{error&&<div className="notice error">{error}</div>}
    {!analysis?<section className="hero"><div className="radar">◎</div><h2>Detect pressure before it cascades.</h2><p>Build a deterministic incident packet here, or invoke <code>$slo-guardian-incident-response</code> in Codex to reason through the local MCP server.</p><button onClick={runAnalysis}>Run demo scenario</button></section>:
    <div className="grid">
